...
public class PretService {
    ...
    public long getNombrePretsEnCoursDomicile(Long adherantId) {
        List<PretModel> prets = pretRepository.findByAdherantId(adherantId);
        return prets.stream()
                .filter(p -> "domicile".equalsIgnoreCase(p.getTypePret().getLibelle()) && p.getDateEffectiveRetour() == null)
                .count();
    }
